...
public class RecyclerViewHandler implements LoadMoreHandler {
    ...
    private RecyclerView getRecyclerView(View contentView){
        RecyclerView result = null;
        if (contentView instanceof RecyclerView){
            result = (RecyclerView) contentView;
        }else {
            for (int i = 0; i < ((ViewGroup) contentView).getChildCount(); i++) {
                View childAt = ((ViewGroup) contentView).getChildAt(i);
                if (childAt instanceof RecyclerView){
                    result = (RecyclerView) childAt;
                }
            }
        }
        return result;
    ...
    public boolean handleSetAdapter(View contentView, ILoadMoreViewFactory.ILoadMoreView loadMoreView, OnClickListener onClickLoadMoreListener) {
        ...
        final RecyclerView recyclerView = getRecyclerView(contentView);
